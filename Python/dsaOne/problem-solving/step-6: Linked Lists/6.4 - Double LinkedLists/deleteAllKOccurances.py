...
class DoublyLinkedList:
    ...
    def deletAllOccurances(self, keyValue):
        if self.head is None:
            return "DLL Empty"
        else:
            if self.head == self.tail and self.head.value == keyValue:
                self.head = None
                self.tail = None
                ...
                currNode = self.head
                while currNode:
                    if self.head.value == keyValue:
                        self.head = self.head.next
                        self.head.prev = None
                    elif self.tail.value == keyValue:
                        self.tail = self.tail.prev
                        self.tail.next = None
                    elif currNode.value == keyValue:
...
#doubleLL.deletAllOccurances(2)
#doubleLL.deletAllOccurances(0)
doubleLL.deletAllOccurances(5)
print([node.value for node in doubleLL]) 
